...
3. **Apply in order.** Each step resolves through the *same* `_apply_action_inner` dispatch + the *same* `_validate_world` gates as a single action does today. All resulting events (and each step's drained relationship shifts) concatenate into ONE `_multi` chain, in execution order.
