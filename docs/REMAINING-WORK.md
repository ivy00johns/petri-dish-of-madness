...
| EM-181 | P2 | W15 | frontend | user 2026-06-11 | Spread sooner: `assignBuildingLots` (cityLayout.ts) assigns new builds to the lots NEAREST each place's landmark block (Euclidean sort), so the city fills center-out and outer blocks (Mossy Row, Rosehip Walk-up) stay empty pads — bias assignment toward under-built blocks/districts so growth distributes across the map. Pairs EM-131 slot placement + EM-174 placement tiers | open | — |
