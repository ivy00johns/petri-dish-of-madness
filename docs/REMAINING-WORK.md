...
| EM-298 | P1 | W30 | backend+frontend | research-v5 (corr. v5-1) | **Agents paint murals, signs & graffiti on surfaces (Rung A).** *Plain:* extend the working plaza-banner image lane to a `paint_surface(target, prompt)` verb → free-first PNG (Pollinations → paid backstop) → three.js decal/atlas tile on a facade, sign, or ground; only a stable `{surface_id→image_id}` enters sim-state (the PNG stays off the replay surface, like today's banner). Per-district decal cap + shared atlas + LRU evict. Cheap, high-spectacle, and **warms up the exact off-replay seam the keystone (EM-299) rides** (v5-1 §3.3 — warm-up before the payoff). **SHIPPED (PR #78, `test_em298_facades.py`); W30 adds a decal-clear follow-up fix (A5) + facades follow-ups filed as EM-302.** Sweep to `COMPLETED-WORK.md` pending (A6 ownership). | done | PR #78 · 2026-07-09 |
...
| EM-300 | P1 | W30 | providers | adaptive-lane-routing spec (docs/superpowers/specs/2026-07-07-adaptive-lane-routing.md) | **Adaptive lane routing initiative record.** P1 (registry + custom sorting list + bounce loop) **SHIPPED PR #83, 2026-07-07**; go-live flip landed in this W30 build. **Open:** P2 dynamic lane discovery/refresh, P3 429-aware cap/cooldown, P4 direct-provider lanes (Gemini/Anthropic/OpenAI/Ollama without the proxy), P5 observability (lane-health surfacing). Spec is the source of truth for scope/sequencing. | open | — |
| EM-301 | P1 | W30 | providers | idle-fallback churn investigation | **Idle-fallback churn thread.** Root cause is intermittent free-tier **request-rate** windows (not the token budget, not a dry pool) compounded by 15–30s proxy timeouts on doomed lanes. **PR #84 (open)** covers timeout labeling + auto-resume. Ollama overflow lane (**EM-167**) is the identified smoothing lever (not yet built). This W30 build adds a sick-pin curated walk + a bounce token-clamp (A8) as an interim mitigation. Deps/relates: EM-300 (P2 discovery would also help), EM-167. | open | — |
