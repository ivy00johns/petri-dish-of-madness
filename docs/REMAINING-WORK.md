...
| EM-297 | P1 | W30 | backend | research-v5 (corr. v5-1) | **World-authorship gate — "do different AIs actually design different buildings?" probe.** *Plain:* before building anything, hand 3–4 free models (gemini-flash/qwen/…) a draft building-recipe format + real prompts and measure whether they emit *varied, coherent* recipes or just echo the example. One-afternoon spike; output is a report, not a game change. **This is the go/no-go for the whole "per-model skyline" thesis** (v5-1 §3.2) — if weak models don't diverge, the keystone's reason to exist evaporates. Needs a tight enum schema + hard validation + sensible defaults regardless. **Blocks EM-299.** | open | — |
