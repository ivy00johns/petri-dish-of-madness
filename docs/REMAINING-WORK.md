...
| EM-239 | P0 | W24 | backend+frontend | layout-spec 2026-06-27 | **S1 — Layout-graph spine (keystone):** replace the frozen `cityLayout.ts` 5×5-grid pure-fn with an event-sourced backend `CityGraph` (nodes/edges/car-policy first-class; lots/zones/landmarks/streets derived); seed via `classic_grid` template; frontend renders FROM the graph (graph-derived road-tile predicate) + derive-on-load migration + graph-less fallback; **byte-identical gate** (EM-155). Unblocks all agent layout control. Spec `docs/superpowers/specs/2026-06-27-agent-city-layout-s1-graph-spine-design.md` | open | — |
...
- **EM-239 + EM-243–248 (Wave N — Agent-Controlled City Layout)** entered 2026-06-27 via
  ...
  **Three locked pillars:** authorship = agents, emergently (backend event-sourced state + verbs);
  ...
  plans/morph). **Sequencing:** EM-239 is the keystone (P0 — everything deps on it); EM-248 (assets)
  is independent and can start anytime; EM-247 gates EM-245 + the geometric EM-246 presets.
