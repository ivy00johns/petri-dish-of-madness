...
| EM-147 | P1 | W14 | backend | wave-c spec C1 | District town config: ~15-place districted `world.yaml` (core/market/residential/civic/farm) + `loader.py` default mirror; additive optional `district` field on `PlaceConfig` + place serialization (default null); e2e test that a ~15-place town runs (movement, building pipeline, well + notice-board resolve). Kinds stay the existing five; lanes derived, not authored. Precursor to EM-123 | open | — |
