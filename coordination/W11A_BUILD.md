...
| Date | Gate | Result |
|---|---|---|
| 2026-06-09 | Wave-1 gate | GREEN — pytest 188/188; `tsc -b` + `vite build` clean; token check on diff clean (hexes = pre-existing in-canvas GPU idiom; style={} = data-driven values only). Scope grew in-wave by user request: EM-102 (label declutter), EM-104 (collapsible legend), EM-105 (resizable feed) + chat-first §9 amendment — all landed. Known carry to wave 2: EM-097 pin must be un-xfailed (now passing); AgentPanels.tsx unused by live route (kept; its test imports it) |
| 2026-06-09 | QA gate (wave 2) | GREEN proceed=true — backend 206/206 (+18 in test_w11a.py), frontend 106/106 (+43 across 6 new test files), EM-097 pin un-xfailed, contract_conformance 5/5, security 4/5, zero blockers; wave 1 src passed every new test on first run. QE findings: INFO narrator skips (not queues) slow windows — in-contract; LOW jsdom can't do pixel-level scroll (covered at gate 3); LOW hook internals untested (data paths covered indirectly) |
| 2026-06-09 | Live verification (gate 3) | GREEN — chat-first layout at 1440×900 (digest + 432px feed left, full-height village, roster strip bottom, controls right); EM-093 measured: 0.00px viewport drift while un-pinned with live arrivals, frozen row set, "↑ 1 new" pill, click-to-repin verified; MODELS (8) legend collapse persisted; resize var + localStorage live; RESET VIEW present; CRITTERS section present; /inspector Run Browser: 27 runs, exactly 1 ACTIVE (26 zombie 'running' rows correctly ARCHIVED), archive mode on run 26 ("Viewing run #26 (archived) · live feed disabled", scrubber max=78), cross-run comparison rendered with AWI indicators; console 0 errors across the whole session. Screenshots: .playwright/2026-06-09-w11a/ |
